...
# Read in tf database file and svm weight file
...
# For wilcox test, if more than 10000 k-mers, take random sample of 10000 
allKmerSample = kmersvmw if kmersvmw.shape[0] < 10000 else kmersvmw.sample(10000)

# Create df of k-mers with their svm weight and motif
kmer_motif = kmer_motif.merge(kmersvmw, how='left', on='kmer')

# Get svm cutoff for top percentile test
...
# Add percentile cutoff column of number of kmers for motif above percentile svm weight
...
# Create dataframe with k-mer, percentile, motif ID, and tf name
motinfo = xn.merge(xn5, how='left', on='Motif_ID')
motinfo = motinfo.merge(TF_motif, how='left', on='Motif_ID')
motinfo= motinfo.fillna(0)

# Calculates the wilcox score for given motif against random sample of all other motifs
...
# Calculate poisson score and adjusted poisson score
...
# Calculate wilcox score and adjusted wilcox score
...
# Change 0 values to minimum float value
...
# Decide whether motif is significant based on cutoffs defined in config file 
...
# Inserts the table data into an HTML file 
...
# If optional tf database metadata file defined in config file
if tf_metadata_fn:

    # Read in metadata file and merge with motif info
    ...
    # Output txt file of all motif info
    ...
    # Calculate fisher exact score for significance of tf families
    ...
    # Output all motif info in text file and HTML file with information just for enriched motifs,
    # without metadata
    filename = f'{outprefix}.all_tfs.txt'
    motinfo.to_csv(filename, sep='\t', index=False, float_format='%.8g')
